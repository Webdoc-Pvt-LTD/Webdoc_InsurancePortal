...
const TelenorSubscriptions = () => {
  ...
  const fetchSubscriptions = async () => {
    ...
    try {
      const response = await fetch(`${Chargingcheck_URL}api/ufone/sub_dump/${msisdn}`);
  ...
  navigate(`/ufone-charging`, { state: { policy } });
};
    ...
    <div className="container-fluid">
      <Card className="shadow-sm">
        <Card.Header className="card-header text-center">
          <h3>Check Ufone Subscriptions</h3>
        </Card.Header>
        ...
        <Card.Body>
          ...
          >
            <InputGroup className="mb-3">
              <Form.Control
                type="text"
                placeholder="Enter MSISDN 3331234567"
                value={msisdn}
